...
#Add quadratic term in the data, also provide the column of ones
modifiedx1 = np.c_[ np.power(x1,2),x1, np.ones(40) ]    
modifiedx2 = np.c_[ np.power(x2,2),x2, np.ones(20) ]   
...
plt.title("Train Data with 2th-order polynomial regression")
plt.ylabel('target')
plt.xlabel('feature')
plt.scatter(x1,y1)
plt.plot(samplepoints,polyModel(samplepoints),'y')
plt.show()
...
plt.title("Test Data with 2th-order polynomial regression")
plt.ylabel('target')
plt.xlabel('feature')
plt.scatter(x2,y2)
plt.plot(samplepoints2,polyModel(samplepoints2),'y')
plt.show()
